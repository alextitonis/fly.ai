...
ADMIN_FEE: constant(uint256) = 0  # 5000000000
...
def remove_liquidity(
    ...
    log RemoveLiquidity(provider=msg.sender, token_amounts=amounts, fees=empty(uint256[N_COINS]), token_supply=total_supply)
...
def remove_liquidity_one_coin(
    ...
    log RemoveLiquidityOne(provider=msg.sender, token_amount=_burn_amount, coin_amount=dy[0], token_supply=total_supply)
...
def ramp_A(_future_A: uint256, _future_time: uint256):
    assert msg.sender == staticcall Factory(self.factory).admin()  # dev: only owner
...
def withdraw_admin_fees():
    receiver: address = staticcall Factory(self.factory).get_fee_receiver(self)

    for i: uint256 in range(N_COINS):
        coin: address = self.coins[i]
        fees: uint256 = staticcall IERC20(coin).balanceOf(self) - self.balances[i]
        raw_call(
            coin,
            concat(
                method_id("transfer(address,uint256)"),
