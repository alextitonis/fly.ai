...
def __init__(_pool: address, _index: uint256, _receiver: address, _caller_share: uint256):
    ...
    POOL = _pool
    I = _index
    pegged: address = staticcall CurvePool(_pool).coins(_index)
    PEGGED = pegged
    extcall ERC20Pegged(PEGGED).approve(_pool, max_value(uint256))
...
def _provide(_amount: uint256):
    extcall ERC20Pegged(PEGGED).mint(self, _amount)

    amounts: uint256[2] = empty(uint256[2])
    amounts[I] = _amount
    extcall CurvePool(POOL).add_liquidity(amounts, 0)
...
def _withdraw(_amount: uint256):
    ...
    amounts: uint256[2] = empty(uint256[2])
    amounts[I] = amount
    extcall CurvePool(POOL).remove_liquidity_imbalance(amounts, max_value(uint256))
...
def _calc_profit() -> uint256:
    lp_balance: uint256 = staticcall CurvePool(POOL).balanceOf(self)

    virtual_price: uint256 = staticcall CurvePool(POOL).get_virtual_price()
...
def update(_beneficiary: address = msg.sender) -> uint256:
    ...
    balance_pegged: uint256 = staticcall CurvePool(POOL).balances(I)
    balance_peg: uint256 = staticcall CurvePool(POOL).balances(1 - I)
    ...
    extcall CurvePool(POOL).transfer(_beneficiary, caller_profit)
...
def withdraw_profit() -> uint256:
    ...
    extcall CurvePool(POOL).transfer(self.receiver, lp_amount)
